...
task :update_recurring_bi_weekly_activities => :environment do
  ...
  Activity.all.each do |activity|
    todays_date = DateTime.now
    if activity.bi_weekly_recurring == "bi-weekly" && activity.start_date < todays_date
